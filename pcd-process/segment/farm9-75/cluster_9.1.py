...
name = f'{stem}_cropx_cropz_cluster_9_1.pcd'

def proc():
  calf = Farm(name, rotate=False, data_path=pcd_path, mkdir=False)
  calf.show_summary()
  #labels = calf.cluster(min_points=3, min_cluster=1000, eps=0.04)
  ...
  min_x = calf.summary['min_bound'][0] + 1.5
  #max_x = min_x
  ...
  labels = calf.cluster(min_points=1, min_cluster=1000, eps=0.07)
  calf.saveClusters_2(labels)
