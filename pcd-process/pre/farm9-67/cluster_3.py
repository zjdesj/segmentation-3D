...
name = f'{stem}_cropx_cropz_cluster_3.pcd'

def proc():
  calf = Farm(name, rotate=False, data_path=pcd_path, mkdir=False)
  calf.show_summary()
  #labels = calf.cluster(min_points=3, min_cluster=2000, eps=0.04)
  ...
  labels = calf.cluster(min_points=3, min_cluster=1000, eps=0.07)
  calf.saveClusters_2(labels)
