...
name = f'{stem}_cropx_cropz_cluster_12.pcd'

def proc():
  calf = Farm(name, rotate=False, data_path=pcd_path, mkdir=False)
  calf.show_summary()
  labels = calf.cluster(min_points=3, min_cluster=1000, eps=0.05)
  calf.saveClusters_2(labels)
  #calf.visual()
  return

  max_y = calf.summary['max_bound'][1] - 0.25
  min_y = calf.summary['min_bound'][1]

  max_x = calf.summary['max_bound'][0]
  min_x = calf.summary['min_bound'][0] + 0.45
  #min_x = max_x - 1.8
  ...
  labels = calf.cluster(min_points=3, min_cluster=1500, eps=0.07)
  calf.saveClusters_2(labels)
